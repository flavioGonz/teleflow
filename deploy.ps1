...
$SSH_HOST = "pbx01.infratec.com.uy"
$SSH_PORT = "11003" # Manteniendo el puerto detectado en el alias local
...
$result = ssh -p $SSH_PORT $SSH_HOST "cd $REMOTE_PATH && git pull origin main 2>&1"
Write-Host $result
...
ssh -p $SSH_PORT $SSH_HOST "chown -R asterisk:asterisk $REMOTE_PATH && chmod -R 755 $REMOTE_PATH" 2>&1
